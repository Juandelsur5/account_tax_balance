...
class WizardOpenTaxBalances(models.TransientModel):
    ...
    date_range_id = fields.Many2one(
        "date.range",
        help="Date range from date_range module. If not available, use from_date and to_date fields."
    )
    ...
    def _compute_date_range(self):
        for wizard in self:
            if wizard.date_range_id:
                try:
                    # Protección defensiva: verificar que el modelo date.range existe
                    # y que los campos date_start y date_end están disponibles
                    if hasattr(wizard.date_range_id, 'date_start') and hasattr(wizard.date_range_id, 'date_end'):
                        wizard.from_date = wizard.date_range_id.date_start
                        wizard.to_date = wizard.date_range_id.date_end
                    else:
                        # Fallback si el modelo existe pero no tiene los campos esperados
                        wizard.from_date = wizard.to_date = None
                except Exception:
                    # Si hay cualquier error al acceder a date_range, usar None
                    wizard.from_date = wizard.to_date = None
            else:
                wizard.from_date = wizard.to_date = None
